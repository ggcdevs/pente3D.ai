...
const TASKS = [
  ...
  },
  {
    ...
    desc:
      ...
      "ASYMMETRY TO CLOSE (from V.4a's review + audit): the peer that is AHEAD detects the divergence and records it, but V.4a deliberately does NOT answer a peer more than one behind — so the BEHIND peer currently learns nothing and sees a normal board. The resolution handshake must therefore be RAISED BY THE DETECTING SIDE and reach the peer, or a divergence stays invisible to exactly the player who is missing moves. Also close V.4a's interim window, recorded in the plan's 'Decisions taken mid-build' section: today a divergence has no session accessor, no window.__pente hook, no UI and no CLI scenario, and the next local move escalates it into a permanent conflict-stop. " +
  ...
  },
  {
    ...
    desc:
      ...
      "(3) RESOLVE THE TWO-WRITER ARRANGEMENT V.1 left behind — this task owns it because it already rewrites main.ts's boot/restore path. Today ONE game can occupy TWO records: the app autosave record (keyed by the app's `autosaveId`) and, once that game is carried into a room, the canonical record under the GAME'S UUID that `NetSession.persistGame` writes. V.1 made the two writers cooperate (a shared `playersFromSeats` projection, `archivedStartedAts` priming so both agree on the date, and a shadow-collapse rule in `listArchivedGames` that hides the non-canonical copy) — machinery that exists only because of the double write. Since #35/S.1 EVERY `Game` already carries a uuid, so the simpler structure is to key EVERY autosave record by `game.uuid` (local games included) and retire `autosaveId` + AUTOSAVE_ID_KEY, which this task is removing from the boot path anyway: then one game has exactly one record, always, and the shadow-collapse rule, the startedAt priming and the cooperating-writers comments all DELETE. THE USER HAS APPROVED THIS REMOVAL (2026-07-25, verbatim: \"yeah, no, remoing that machinery seems good :)\"), so it is a REQUIREMENT of this task, not an option: key EVERY autosave record by `game.uuid`, retire `autosaveId` + AUTOSAVE_ID_KEY, and DELETE the machinery that existed only to reconcile two writers — the shadow-collapse rule in `listArchivedGames`, the `archivedStartedAts` priming if it becomes unnecessary, and the cooperating-writers commentary in main.ts/session.ts. Expect a NET REMOVAL of code; report the line delta. Keep the archive MIGRATION honest: records already keyed by an old autosaveId must still be listed and resumable (they carry `meta.uuid`), so nobody loses a game to the re-keying. If some specific part genuinely cannot go, name it and say why — but \"document why not\" is not a way out of the removal. V.6 must inherit ONE record per game, not a listing heuristic. " +
