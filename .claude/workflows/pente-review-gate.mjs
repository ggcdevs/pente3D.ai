...
const gate = await agent(
  `VERIFICATION GATE for Pente3D Stage ${STAGE}. Run everything IN ${REPO} (that exact worktree — this repo has several on different branches, and this gate PUSHES). Work in-place on the branch it has checked out; do NOT \`git checkout\`/\`git switch\`. ${DOCTRINE}\n` +
    ...
    `6. \`npm run scenario:all\` — the WHOLE CLI matrix over the real relay (issue45, mirror, divergence, code-reuse, rematch, both-absent, ff-boundary, …), not just the one scenario this gate used to run. A matrix only does its job if it is all run, routinely; gating on one member let the other seven rot unnoticed. THREE distinct outcomes, do not conflate them: exit 0 = a reconnecting peer converges to the live game (the v3.1 acceptance test, required from task V.3 onward); exit 2 = SKIPPED, relay unreachable (missing egress, NOT a regression — report it as skipped and do not block on it); exit 1 = checks FAILED, i.e. the bug is present. Report the exit code and the failing check lines verbatim.\n` +
    ...
    `passed = lint(0) AND all tests pass AND coverage 100% AND mutation >= ${MUT_MIN}% (survivors justified) AND typecheck:cli 0 AND scenario:all in {0, 2} (2 = every scenario skipped for lack of egress; ANY failing scenario is exit 1) once V.3 has landed. Report the per-scenario pass/FAIL/skipped lines verbatim — a suite line alone hides which member regressed.\n` +
