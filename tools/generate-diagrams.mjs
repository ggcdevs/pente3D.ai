...
// Regexes (matched against the absolute file path) for files to omit from the
// diagrams. Defaults to test/spec files so the architecture view isn't dominated
// by test scaffolding and test→source edges. Configurable via `excludePatterns`.
const EXCLUDE_PATTERNS = (config.excludePatterns || ['\\.test\\.', '\\.spec\\.'])
    .map(p => new RegExp(p));
    ...
    for (const entry of readdirSync(dir)) {
        ...
        if (stat.isDirectory()) {
            results.push(...findSourceFiles(full));
                   ...
                   && !EXCLUDE_PATTERNS.some(re => re.test(full))) {
            results.push(full);
        }
    }
    return results;
...
function main() {
    ...
    // --force is the default behavior in non-check mode (files are always rewritten),
    // so it needs no dedicated branch — it exists as an explicit, self-documenting alias.
    ...
    if (checkMode) {
        ...
        // SVG rendering is opt-in via `renderSvg: true` in diagrams.json. Left off by
        // default so no @mermaid-js/mermaid-cli (puppeteer/chromium) dependency or `npx`
        // network fetch is required — GitHub renders the committed .mmd natively.
        if (!config.renderSvg) {
            success('All .mmd diagrams generated! (SVG rendering disabled; set "renderSvg": true in diagrams.json to enable)');
            return;
        }
