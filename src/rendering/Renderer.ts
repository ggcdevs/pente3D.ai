...
export class Renderer {
  ...
  private highlightedPieces: Map<string, { mesh: THREE.Mesh; originalMaterial: THREE.Material }> =
    new Map();
  private nodeHighlights: Map<string, { mesh: THREE.Mesh; originalMaterial: THREE.Material }> =
    new Map();

  // State indicators
  private currentPlayerIndicator: THREE.Mesh | null = null;
  private captureCountSprites: { black: THREE.Sprite | null; white: THREE.Sprite | null } = {
    black: null,
    white: null,
  };
  ...
  private initializeGeometries(): void {
    ...
    this.pieceGeometry = new THREE.SphereGeometry(this.options.pieceSize, 16, 16);

    // Node geometry (small sphere)
    this.nodeGeometry = new THREE.SphereGeometry(this.options.pieceSize * 0.2, 8, 8);
  }
  ...
  private createBoardGrid(): void {
    ...
    gridLines.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
  ...
  private createPieceMesh(piece: Piece): THREE.Mesh {
    ...
    if (piece.isTemporary) {
      material =
        piece.player.color === 'black' ? this.temporaryBlackMaterial : this.temporaryWhiteMaterial;
    } else {
      material = piece.player.color === 'black' ? this.blackPieceMaterial : this.whitePieceMaterial;
    }
  ...
  setTemporaryPiece(position: Vector3, player: Player): void {
    ...
    // Add outline effect
    const outlineGeometry = new THREE.SphereGeometry(this.options.pieceSize * 1.1, 16, 16);
  ...
  updateCaptureCount(blackCaptures: number, whiteCaptures: number): void {
    ...
    this.captureCountSprites.black = createTextSprite(`Black: ${blackCaptures}`, '#ffffff');
    this.captureCountSprites.black.position.set(-boardExtent, 0, 0);

    this.captureCountSprites.white = createTextSprite(`White: ${whiteCaptures}`, '#ffffff');
  ...
  startRenderLoop(): void {
    ...
    const animate = () => {
      ...
      if (
        this.temporaryPiece &&
        this.temporaryPiece.userData.time !== undefined &&
        !this.reducedMotion
      ) {
  ...
  render(): void {
    ...
    if (this.lodManager.frameCounter % 2 === 0) {
      // Update frustum every other frame
  ...
  focusCameraOnPosition(position: Vector3): void {
    ...
    if (cameraDistance > this.options.boardSize * 2) {
      ...
      const newPosition = targetPosition
        .clone()
        .add(direction.multiplyScalar(this.options.boardSize * 1.5));
